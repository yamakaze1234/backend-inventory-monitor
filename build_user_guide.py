...
def build(destination, screenshot=None):
    ...
    page='<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>后台库存监控 v1.01 · 完整使用教程</title><style>'+css+'</style></head><body><aside><b>后台库存监控</b><small>完整使用教程 · v1.01<br>采集脚本 0.4.0</small><nav>'+''.join(toc)+'</nav></aside><main><header class="hero"><div class="eyebrow">安装 · 配置 · 使用 · 故障处理</div><h1>从第一次连接<br>到本机自动重登</h1><p class="lead">先完成库存采集，再按需开启机器人、钉钉监听和无人值守。本文明确区分实测结果、环境要求与尚未验证的边界。</p><div class="pills"><span>Windows 11 x64 已实测</span><span>Win7 不支持</span><span>本机执行，无需云端 AI</span></div><div class="tools"><button onclick="window.print()">打印 / 保存 PDF</button><span>双击即可离线阅读 · Ctrl+F 查找 · 更新于 2026-09-13</span></div></header>'+picture+''.join(content)+'</main></body></html>'
